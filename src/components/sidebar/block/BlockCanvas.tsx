...
const BlockCanvas = ({ colors, selectedColor, onCellClick = () => { }, editable = false }:
  ...
  function handleCellMouseDown(e: React.MouseEvent<HTMLDivElement, MouseEvent>, i: number) {
    e.preventDefault()
  ...
  return (
    <SimpleGrid columns={10} border={"3px solid gray"}>
      {
        colors.map((color, i) => <Box border={"1px solid gray"} key={i} bg={color} height='32px' width='32px' onMouseEnter={(e) => handleCellMouseDown(e, i)} onClick={(e) => handleCellClick(e, i)} />)
      }
    </SimpleGrid>)
}
